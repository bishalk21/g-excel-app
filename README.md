...
- cycle detection in directed graphs for handling cell dependencies and recalculations when cell values change, using a graph algorithm to identify and manage circular references in formulas, ensuring that the application can handle complex dependencies between cells without crashing or producing incorrect results.
  ...
  - Cells tracking in cycle detection:
    - Color Tracking Cyclic Path (Detect cycle algo): When performing a depth-first search (DFS) to detect cycles in a directed graph, we can use color tracking to keep track of the state of each node (cell) during the traversal. The common approach is to use three colors:
      - White (0): The node has not been visited yet.
      - Gray (1): The node is currently being visited (i.e., it is in the recursion stack).
      - Black (2): The node has been fully processed and is no longer in the recursion stack.
    - delay (color change) in cycle detection: When we detect a cycle during the DFS traversal, we can delay the color change of the nodes involved in the cycle until we have completed the traversal of that path. This allows us to identify all nodes that are part of the cycle before marking them as black (fully processed), ensuring that we can handle circular references effectively without prematurely marking nodes as processed.
      - setTimeOut() can be used to delay the color change of nodes involved in a detected cycle, allowing us to complete the traversal and identify all nodes in the cycle before marking them as processed. This approach helps to manage circular references in formulas without causing crashes or incorrect results in the spreadsheet application.
    - wait (sync) in cycle detection: In some cases, we may want to wait for the completion of the DFS traversal before marking nodes as processed, especially when dealing with complex dependencies and circular references. This can be achieved using synchronous code execution, ensuring that we have fully traversed the graph and identified all nodes involved in a cycle before updating their state. This approach helps to maintain the integrity of the dependency graph and ensures accurate results in the spreadsheet application when handling circular references.
      - Promises/(async/await) can be used to wait for the completion of the DFS traversal before marking nodes as processed, allowing us to handle complex dependencies and circular references effectively in the spreadsheet application.
